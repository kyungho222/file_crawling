...
try:
    ...
    if not env_loaded:
        print("[SETTINGS] WARNING: .env 파일을 찾을 수 없거나 로드하지 못했습니다.", flush=True)
    ...
    load_dotenv = None
    print(f"[SETTINGS] WARNING: .env bootstrap skipped: {e}", flush=True)
...
class Settings:
    ...
    DOWNLOAD_WORKERS: int = _env_int("DOWNLOAD_WORKERS", _env_int("FILE_CRAWL_DOWNLOAD_WORKERS", 5, max_value=16), max_value=16)
    FILE_CRAWL_LARGE_DOWNLOAD_WORKERS: int = _env_int(
        "FILE_CRAWL_LARGE_DOWNLOAD_WORKERS", 2, max_value=16
    )
    ...
    DOWNLOAD_MAX_CONCURRENT: int = _env_int("DOWNLOAD_MAX_CONCURRENT", _env_int("FILE_CRAWL_DOWNLOAD_MAX_CONCURRENT", 5, max_value=16), max_value=16)
    ...
    BOARD_CONTENT_DETAIL_CONCURRENCY: int = 8
    BOARD_CONTENT_PIPELINE_DETAIL_CONCURRENCY: int = 8
    ...
    URL_GLOBAL_CONCURRENT_LIMIT: int = int(os.getenv("URL_GLOBAL_CONCURRENT_LIMIT", "8"))
    ...
    for path in candidates:
        try:
            if path and path.exists():
                if _safe_load_dotenv(path, override=False):
                    return str(path)
        except Exception:
            continue
...
async def connect_db(dbname=None):
    ...
    Args:
        dbname (str, optional): 연결할 데이터베이스 이름.
...
async def return_connection(conn, dbname=None):
    ...
    try:
        if conn:
            ...
            if pool is None:
                pool = await DatabasePool.get_pool(dbname)
            await pool.release(conn)  # 올바른 방식 (닫는 것이 아니라 반환)
    except Exception as e:
        msg = str(e)
