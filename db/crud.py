...
def get_session_by_job_id(db: Session, job_id: str) -> Optional[CrawlSession]:
    ...
    logger.debug("=" * 60)
    logger.debug("🔍 [DB 조회] get_session_by_job_id")
    ...
    try:
        ...
        if result:
            logger.debug(f"   ✅ 세션 조회 성공: session_id={result.id}")
            logger.debug(f"      • status: {result.status}")
            logger.debug(f"      • chat_bot_id: {getattr(result, 'chat_bot_id', 'N/A')}")
            logger.debug(f"      • subject: {getattr(result, 'subject', 'N/A')}")
            logger.debug(f"      • scan: {getattr(result, 'scan', 0)}")
            logger.debug(f"      • collection: {getattr(result, 'collection', 0)}")
            logger.debug(f"      • save: {getattr(result, 'save', 0)}")
        else:
...
def update_session_on_download_complete(
    ...
    # 업데이트
    # 완료 처리 직전에 블로킹 짧은 지연을 둔다 (동기 DB 함수)
    try:
        time.sleep(3)
    except Exception:
        pass
...
def bulk_create_files(db: Session, files_data: List[dict], batch_size: int = 100) -> tuple[List[CrawledFile], int]:
    ...
    for batch_start in range(0, total_files, batch_size):
        ...
        try:
            ...
            if batch_end % (batch_size * 10) == 0 or batch_end == total_files:
                import logging
                logger = logging.getLogger(__name__)
                logger.debug(f"💾 배치 저장 진행: {batch_end}/{total_files}개 ({batch_end*100//total_files if total_files > 0 else 0}%)")
        except Exception as e:
            db.rollback()
            import logging
            logger = logging.getLogger(__name__)
    ...
    if skipped_count > 0:
        import logging
        logger = logging.getLogger(__name__)
        logger.debug(f"ℹ️ 중복 URL 건너뜀: {skipped_count}개")
...
def get_files_by_session_id(db: Session, session_id: int) -> List[CrawledFile]:
    ...
    logger.debug("=" * 60)
    logger.debug("🔍 [DB 조회] get_files_by_session_id")
    ...
    try:
        ...
        logger.debug(f"   ✅ 파일 목록 조회 성공: {len(result)}개")
        
        # 다운로드 완료 파일 통계
        downloaded_count = sum(1 for f in result if getattr(f, 'is_downloaded', False))
        logger.debug(f"      • 전체 파일: {len(result)}개")
        logger.debug(f"      • 다운로드 완료: {downloaded_count}개")
        logger.debug(f"      • 다운로드 미완료: {len(result) - downloaded_count}개")
        
        # 처음 5개 파일 정보 출력
        if result:
            logger.debug("      📁 파일 목록 (처음 5개):")
            for idx, f in enumerate(result[:5], 1):
                logger.debug(f"         [{idx}] {f.filename}")
                logger.debug(f"             • URL: {f.url[:80] if f.url else 'N/A'}...")
                logger.debug(f"             • 다운로드: {'✅' if getattr(f, 'is_downloaded', False) else '❌'}")
                logger.debug(f"             • 경로: {getattr(f, 'local_path', 'N/A')}")
            if len(result) > 5:
                logger.debug(f"         ... 외 {len(result) - 5}개")
