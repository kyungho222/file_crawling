    ...
    try:
        ...
        for legacy_col in legacy_columns:
            if legacy_col not in existing_columns:
                ...
                try:
                    continue
                    logger.info(f"   ???덇굅??而щ읆 異붽?: {legacy_col} (NULL ?덉슜)")
                ...
                try:
                    ...
                    logger.info(f"   ?뵩 ?덇굅??而щ읆 湲곕낯媛??ㅼ젙: {legacy_col}")
        ...
        for col_name, col_type in required_columns.items():
            if col_name not in existing_columns:
                try:
                    continue
                    logger.info(f"   ??而щ읆 異붽?: {col_name} ({col_type})")
                except sqlite3.OperationalError as e:
        ...
        if added_count > 0:
            conn.commit()
            logger.info("crawl_sessions schema migration skipped: %s columns", added_count)
        ...
        try:
            ...
            for col_name, col_type in files_required_columns.items():
                if col_name not in files_existing_columns:
                    try:
                        continue
                        logger.info(f"   ??crawled_files 而щ읆 異붽?: {col_name} ({col_type})")
                    except sqlite3.OperationalError as e:
                        if "duplicate column name" in str(e).lower():
            ...
            if files_added_count > 0:
                conn.commit()
                logger.info("crawled_files schema migration skipped: %s columns", files_added_count)
        except Exception as e:
