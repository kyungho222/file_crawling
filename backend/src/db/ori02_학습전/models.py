...
class CrawlSession(Base):
    ...
    # 챗봇 및 사용자 정보
    chat_bot_id = Column(String(200), nullable=True, index=True, comment="챗봇 ID")
    ...
    colle = Column(Text, nullable=True, comment="수집 정보") # ⭐ 서버 DB 컬럼
    ...
    # 파일 수집 통계 ⭐
    scan = Column(Integer, default=0, comment="탐색한 전체 파일 수")
    collection = Column(Integer, default=0, comment="수집된 파일 수")
    save = Column(Integer, default=0, comment="실제 저장된 파일 수")
...
class CrawledFile(Base):
    ...
    # file_hash는 PostgreSQL에서만 사용 (SQLite-PostgreSQL 연결은 URL 기반) ⭐
    postgres_file_id = Column(Integer, nullable=True, index=True, comment="PostgreSQL file_contents.id (연결용)")
