...
def create_session(
    ...
    scan: int = 0,  # 수집 예정 파일 개수
    collection: int = 0,  # 수집된 파일 개수
    save: int = 0,  # 실제 다운로드된 파일 개수
    pages: int = 0  # 크롤링한 페이지 수
) -> CrawlSession:
    """크롤링 세션 생성 (서버 DB 스키마에 맞춤)"""
    import logging
    ...
    db_session = CrawlSession(
        ...
        save=save,
        pages=pages
        # start_url, max_depth, max_pages, user_agent는 서버 DB에 없으므로 제외
    )
