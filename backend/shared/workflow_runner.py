    ...
    if _distributed_duplicate_workflow_lock_enabled() and not dashboard_job and not local_file_crawl_job:
        ...
        try:
            ...
            if not distributed_lock_acquired:
                ...
                if (
                    existing_job_id
                    and existing_job_id != str(job_id or "").strip()
                    and not _duplicate_lock_blocks_different_job_ids()
                ):
                    logger.warning(
                        "[RunWorkflowTask][DuplicateLock] same fingerprint but different job_id; allow concurrent run | "
                        "job_id=%s existing_job_id=%s key=%s start_urls=%s db=%s ttl=%s active=%s",
                        job_id,
                        existing_job_id,
                        distributed_lock_key,
                        len(start_urls or []),
                    ...
                    distributed_lock_key = f"{distributed_lock_key}:job:{job_id}"
                    distributed_lock_acquired = bool(await redis.set(distributed_lock_key, job_id, ex=ttl, nx=True))
                    if not distributed_lock_acquired:
                        logger.warning(
                            "[RunWorkflowTask][DuplicateLock] job-scoped duplicate blocked | job_id=%s key=%s start_urls=%s db=%s ttl=%s",
                            job_id,
                            distributed_lock_key,
                            len(start_urls or []),
                            db_name,
                        ...
                        duplicate_payload = {
                            "status": "duplicate",
                            "event": "duplicate_workflow_blocked",
                            "job_id": job_id,
                            ...
                            "message": "揶쏆늿? job_id????덉뵬 ??곌쾿???쨮???遺욧퍕????? ??쎈뻬 餓λ쵐???덈뼄.",
                        }
                        try:
                            await update_state_only(job_id=job_id, account_name=db_name, payload=duplicate_payload)
                        except Exception:
                            pass
                        try:
                            enqueue_sse_message(
                    ...
                    owner_snapshot = await _inspect_duplicate_lock_owner(
                        redis=redis,
                        db_name=db_name,
                        lock_key=distributed_lock_key,
                        lock_ttl=ttl,
                        existing_job_id=existing_job_id,
                    )
                if _duplicate_lock_stale_release_enabled() and owner_snapshot.get("stale"):
                    try:
                        owner_now = await redis.get(distributed_lock_key)
                        if isinstance(owner_now, (bytes, bytearray)):
                            owner_now = owner_now.decode("utf-8", errors="replace")
                        if str(owner_now or "").strip() == existing_job_id:
                            await redis.delete(distributed_lock_key)
                            logger.warning(
                                "[RunWorkflowTask][DuplicateLock] stale lock released | job_id=%s existing_job_id=%s reason=%s snapshot=%s",
                                job_id,
                                existing_job_id,
                                owner_snapshot.get("stale_reason"),
                                owner_snapshot,
                            )
                            distributed_lock_acquired = bool(
                                await redis.set(distributed_lock_key, job_id, ex=ttl, nx=True)
                            )
                            if not distributed_lock_acquired:
                                raw_existing = await redis.get(distributed_lock_key)
                                if isinstance(raw_existing, (bytes, bytearray)):
                        ...
                        logger.warning(
                            "[RunWorkflowTask][DuplicateLock] stale lock release failed | job_id=%s existing_job_id=%s err=%s snapshot=%s",
                            job_id,
                            existing_job_id,
                            stale_exc,
                            owner_snapshot,
                ...
                if not distributed_lock_acquired:
                    logger.warning(
                        "[RunWorkflowTask][DuplicateLock] duplicate workflow blocked | job_id=%s existing_job_id=%s start_urls=%s db=%s ttl=%s owner=%s",
                        job_id,
                        existing_job_id,
                        len(start_urls or []),
                        db_name,
                    ...
                    duplicate_payload = {
                        ...
                        "study_count": 0,
                        "timestamp": datetime.now().isoformat(),
                        "message": "揶쏆늿? ??곌쾿???쨮???遺욧퍕????? ??쎈뻬 餓λ쵐???餓λ쵎???臾믩씜??筌△뫀???됰뮸??덈뼄.",
                    }
                    try:
                        await update_state_only(job_id=job_id, account_name=db_name, payload=duplicate_payload)
                    except Exception:
                    ...
                    try:
                        crawler_state.record_history(
                            job_id,
                            "duplicate",
                            f"duplicate_of:{existing_job_id}",
                            db_name,
                            chat_bot_id=chat_bot_id,
                        )
                    except Exception:
            ...
            if False:
                logger.warning(
                    "[RunWorkflowTask][DuplicateLock] duplicate workflow blocked | job_id=%s existing_job_id=%s start_urls=%s db=%s ttl=%s",
                    job_id,
                    existing_job_id,
                    len(start_urls or []),
                    db_name,
                ...
                duplicate_payload = {
                    ...
                    "study_count": 0,
                    "timestamp": datetime.now().isoformat(),
                    "message": "揶쏆늿? ??쨌筌??遺욧퍕????? ??쎈뻬 餓λ쵐???餓λ쵎???臾믩씜??筌△뫀???됰뮸??덈뼄.",
                }
                try:
                    await update_state_only(job_id=job_id, account_name=db_name, payload=duplicate_payload)
                except Exception:
